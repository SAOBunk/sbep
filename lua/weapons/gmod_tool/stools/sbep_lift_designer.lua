...
local PartModels = {
	["smallbridge"] = {
		["h"]="models/smallbridge/station parts/sbhubl.mdl",
		["m"]="models/smallbridge/elevators_small/sbselevm.mdl",
		["me"]="models/smallbridge/elevators_small/sbselevte.mdl",
		["medh"]="models/smallbridge/elevators_small/sbselevmedh.mdl",
		["medw"]="models/smallbridge/elevators_small/sbselevmedw.mdl",
		["mr"]="models/smallbridge/elevators_small/sbselevmr.mdl",
		["mt"]="models/smallbridge/elevators_small/sbselevmt.mdl",
		["mv"]="models/smallbridge/station parts/sbbridgevisorm.mdl",
		["mx"]="models/smallbridge/elevators_small/sbselevmx.mdl",
		["s"]="models/smallbridge/elevators_small/sbselevs.mdl"
	},
	["modbridge"] = {
		["m"] = "models/cerus/modbridge/misc/elevator/ecb111.mdl",
		["me"] = "models/cerus/modbridge/misc/elevator/eb111.mdl",
		["mv"] = "models/cerus/modbridge/core/s-331eb.mdl",
		["s"] = "models/cerus/modbridge/misc/elevator/etm113.mdl"
	}
}

local Music = {
	["music/elevator-bossa-nova.wav"] = "Generic",
	["ambient/music/looping_radio_mix.wav"] = "Portal Radio",
	["music/conga_sketch_167bpm_01-04.wav"] = "TF2 Conga",
	["music/mannrobics.wav"] = "TF2 Mannrobics",
	["music/cossack_sandvich.wav"] = "TF2 Kazotsky",
	["music/hl1_song25_remix3.mp3"] = "Half Life Credits",
	["music/hl2_song20_submix0.mp3"] = "HL2 CP Violation",
	["music/radio1.mp3"] = "HL2 Radio 1",
	["ambient/music/cubanmusic1.wav"] = "CSS Cuban",
	["ambient/music/mirame_radio_thru_wall.wav"] = "Portal Distant Mirame"
}
...
if CLIENT then
	...
	function CreateSBEPLiftDesignerMenu()
		...
		for k,v in ipairs(BMT) do
			if PartModels[string.lower(Set)][string.lower(v.type)] then
				LDT.PButtons.Part[k] = vgui.Create( "SpawnIcon" , LDT.Frame ) -- SpawnIcon
				LDT.PButtons.Part[k]:SetPos( 5 + 80 * ((k - 1)%4) , 28 + 80 * math.floor((k - 1)/ 4) )
				LDT.PButtons.Part[k]:SetSize( 75 , 75 )
				LDT.PButtons.Part[k]:SetModel( PartModels[string.lower(Set)][string.lower(v.type)] )
		...
		-- if file.Exists( "materials/sbep_icons/"..Set.."/h.vmt", "GAME" ) then
			-- LDT.PButtons.Special.Part[2] = vgui.Create("DImageButton", LDT.SFrame )
				...
				-- LDT.PButtons.Special.Part[2].DoClick = function()
													...
													-- end
		-- end
		if PartModels[string.lower(Set)]["mv"] then
			LDT.PButtons.Special.Part[1] = vgui.Create( "SpawnIcon" , LDT.SFrame ) -- SpawnIcon
			LDT.PButtons.Special.Part[1]:SetPos( 5 , 5 )
			LDT.PButtons.Special.Part[1]:SetSize( 75 , 75 )
			LDT.PButtons.Special.Part[1]:SetModel( PartModels[string.lower(Set)]["mv"] )
			LDT.PButtons.Special.Part[1].DoClick = function()
				LDT.SFrame.visible = false
				LDT.SFrame:SetVisible( LDT.SFrame.visible )
				RunConsoleCommand( "SBEP_LiftSys_SetLiftPartType_ser" , SMT[1].type )
		...
		if PartModels[string.lower(Set)]["h"] then
			LDT.PButtons.Special.Part[2] = vgui.Create( "SpawnIcon" , LDT.SFrame ) -- SpawnIcon
			LDT.PButtons.Special.Part[2]:SetPos( 85 , 5 )
			LDT.PButtons.Special.Part[2]:SetSize( 75 , 75 )
			LDT.PButtons.Special.Part[2]:SetModel( PartModels[string.lower(Set)]["h"] )
			LDT.PButtons.Special.Part[2].DoClick = function()
				LDT.SFrame.visible = false
				LDT.SFrame:SetVisible( LDT.SFrame.visible )
				RunConsoleCommand( "SBEP_LiftSys_SetLiftPartType_ser" , SMT[2].type )
...
function TOOL:LeftClick( trace )
	...
	if Editing == 0 then
		...
		local startpos = trace.HitPos
			...
			LiftSystem_SER[ply].Soundfont = ply:GetInfo( "sbep_lift_designer_soundfont" )
		...
		LiftSystem_SER[ply].UsableElev = ply:GetInfoNum( "sbep_lift_designer_enableuse_elevator", 0 ) == 1
		LiftSystem_SER[ply].Music = ply:GetInfo( "sbep_lift_designer_musicpath" ) ~= "None"
		LiftSystem_SER[ply].MusicPath = ply:GetInfo( "sbep_lift_designer_musicpath" )
...
function TOOL.BuildCPanel(panel)
	...
	SoundfontListView = vgui.Create("DListView")
		SoundfontListView:SetSize(100, 101)
		SoundfontListView:SetMultiSelect(false)
		SoundfontListView:AddColumn("Soundfont")
		SoundfontListView.OnRowSelected = function(self, id, line)
												self:ClearSelection()
												line:SetSelected( true )
												RunConsoleCommand( "sbep_lift_designer_soundfont", line:GetValue(1) )
										end
		SoundfontListView:AddLine("Scrappers")
		SoundfontListView:AddLine("Advanced")
		SoundfontListView:AddLine("SlyBridge")
		SoundfontListView:AddLine("MedBridge2")
		SoundfontListView:AddLine("Jaanus")
		SoundfontListView:AddLine("Modbridge")
		SoundfontListView:GetLine( 1 ):SetSelected( true )
		RunConsoleCommand( "sbep_lift_designer_soundfont", "Scrappers" )
	panel:AddItem( SoundfontListView )
	
	--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	
	MusicListView = vgui.Create("DListView")
		MusicListView:SetSize(100, 101)
		MusicListView:SetMultiSelect(false)
		MusicListView:AddColumn("Music")
		MusicListView.OnRowSelected = function(self, id, line)
												self:ClearSelection()
												line:SetSelected( true )
												RunConsoleCommand( "sbep_lift_designer_musicpath", line:GetValue(2) )
										end
		MusicListView:AddLine("None", "None")
		for k,v in pairs(Music) do
			MusicListView:AddLine(v, k)
		end
		MusicListView:GetLine( 1 ):SetSelected( true )
		RunConsoleCommand( "sbep_lift_designer_musicpath", "None" )
	...
	local UseCheckBox = vgui.Create( "DCheckBoxLabel" )
		UseCheckBox:SetText( "Enable Use Key on Elevator" )
		UseCheckBox:SetTextColor(Color(0,0,0,255))
		UseCheckBox:SetConVar( "sbep_lift_designer_enableuse_elevator" )
		UseCheckBox:SetValue( 0 )
		UseCheckBox:SizeToContents()
	panel:AddItem( UseCheckBox )
